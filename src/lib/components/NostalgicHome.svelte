...
<div class="nostalgic-home">
	...
	<section class="box recent">
		<h2>最近の記録</h2>
		<ul>{#each records as record,index}<li><time datetime={record.date}>{record.dateLabel}</time><span>制作記録</span><a href={record.url}>{record.title}</a>{#if index===0}<b>NEW</b>{/if}</li>{/each}</ul>
...
<style>
	...
	@media(max-width:620px){.nostalgic-home{padding:.1rem .65rem 1rem}.welcome{padding:.45rem 0 .75rem}.welcome img{width:78%}.box{padding:.7rem;box-shadow:3px 3px 0 rgba(90,101,177,.2)}.work-grid{grid-template-columns:1fr 1fr;grid-template-rows:150px 110px 110px}.work-grid>a.large{grid-column:1/3;grid-row:auto}.work-grid>a:last-child{grid-column:1/3}li{grid-template-columns:5rem 4rem 1fr}li b{display:none}}
</style>
