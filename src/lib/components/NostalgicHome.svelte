...
<div class:with-photos={photos.length > 0} class="home-stage">
	...
	<section class="box">
		<h1>WORKS</h1>
		<div class="work-grid">{#each works.slice(0,4) as work,index}{#if work.externalUrl}<a class="tile" class:large={index===0} href={work.externalUrl} target="_blank" rel="noopener noreferrer">{#if work.thumbnail}<img src={work.thumbnail} alt="" />{/if}<span><strong>{work.title}</strong><time datetime={work.date}>{work.dateLabel}</time></span></a>{:else}<div class="tile" class:large={index===0}>{#if work.thumbnail}<img src={work.thumbnail} alt="" />{/if}<span><strong>{work.title}</strong><time datetime={work.date}>{work.dateLabel}</time></span></div>{/if}{/each}</div>
