...
<style>
	.note-card { display: flex; flex-direction: column; gap: .75rem; min-width: 0; padding: 1rem; border: 1px solid var(--border); border-radius: var(--radius-lg); color: var(--text); background: var(--card); box-shadow: var(--shadow-sm); text-decoration: none; transition: transform .15s ease, box-shadow .15s ease, border-color .15s ease; }
	...
	@media (prefers-reduced-motion: reduce) {
		.note-card, .photo { transition: none; animation: none; transform: rotate(var(--tilt)); }
	}
</style>
