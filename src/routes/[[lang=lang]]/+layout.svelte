<script lang="ts">
	...
	let { data, children }: { data: { railNews: ContentSummary[]; lang: Lang }; children: Snippet } = $props();
...
<svelte:head>
	...
	<link rel="preload" href={assetPath('/header.webp')} as="image" type="image/webp" />
...
<header class="site-header">
	<a class="site-title" href={path('/')} aria-label={strings.nav.homeAria} onclick={releaseHeaderWind}>
		<img class:spinning={navigating} class="site-icon" src={assetPath('/favicon.svg')} alt="" width="44" height="44" />
	...
	<div class="header-actions">
		...
		<nav class="desktop-nav" aria-label={strings.nav.mainNavAria}>
			<a class="nav-works" data-en="works" class:active={currentSection === 'works'} aria-current={currentSection === 'works' ? 'page' : undefined} href={path('/works/')}>{strings.nav.works}</a>
		...
		<div class="menu-shell" bind:this={menuShell}>
			...
			{#if menuOpen}
				<nav id="site-menu" class="site-menu" aria-label={strings.nav.menuAria}>
					...
					<a class="menu-records" class:active={currentSection === 'records'} aria-current={currentSection === 'records' ? 'page' : undefined} href={path('/records/')}><span>{@render menuIconGallery()}</span><strong>{strings.nav.records}</strong><small>{strings.nav.recordsDesc}</small></a>
					<a class="menu-news" class:active={currentSection === 'news'} aria-current={currentSection === 'news' ? 'page' : undefined} href={path('/news/')}><span>{@render menuIconNews()}</span><strong>{strings.nav.news}</strong><small>{strings.nav.newsDesc}</small></a>
					<a class="menu-about" class:active={currentSection === 'about'} aria-current={currentSection === 'about' ? 'page' : undefined} href={path('/about/')}><span>{@render menuIconAbout()}</span><strong>{strings.nav.about}</strong><small>{strings.nav.aboutDesc}</small></a>
				</nav>
			{/if}
		</div>
	</div>
...
<footer><small>© {new Date().getFullYear()} かざぐるま</small><nav aria-label={strings.footer.externalLinksAria}><a href="https://www.youtube.com/@Kazashimo_Ch" target="_blank" rel="noopener noreferrer">YouTube</a><a href="https://x.com/haru01234567890" target="_blank" rel="noopener noreferrer">X</a><span>{strings.footer.contactSoon}</span></nav></footer>
